...
struct Sample {
    uint64_t timestamp;
    std::array<short, NUM_TEMP_PROBES> temperatures;
    short flowRate;
};
...
typedef struct tempProbe {
    ...
    static std::array<tempProbe, NUM_TEMP_PROBES> probes;
}tempProbe;
...
typedef struct flowMeter {
    ...
    short getFlowRate();                //Reads the flow meter and returns the flow rate
